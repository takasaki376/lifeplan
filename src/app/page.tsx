                    ...
                    <Tabs defaultValue="table">
                      ...
                      <Tabs.Panel value="table" pt="md">
                        ...
                        <ScrollArea h={showInputForm ? 400 : 600}>
                          <Table striped highlightOnHover stickyHeaderOffset={1}>
                            <Table.Thead>
                              <Table.Tr>
                                <Table.Th>年</Table.Th>
                                <Table.Th>年齢</Table.Th>
                      ...
                      <Tabs.Panel value="chart" pt="md">
                        <Text size="sm" c="dimmed" mb="lg">
                          ライフプラン推移グラフ（収入・支出：棒グラフ、総資産：折れ線グラフ）
                        </Text>
                        <div style={{ height: showInputForm ? 400 : 600 }}>
                          <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart
                              data={lifePlanResults.map((result) => ({
                                age: result.age,
                                year: result.year,
                                totalAssets: result.totalAssets,
                                totalIncome: result.totalIncome,
                                totalExpenses: -result.totalExpenses, // 支出をマイナス値として表示
                              }))}
                              margin={{
                                top: 20,
                                right: 30,
                              ...
                              <Tooltip
                                formatter={(value, name) => {
                                  if (name === 'totalExpenses') {
                                    return [formatNumber(Math.abs(Number(value))), '年支出'];
                                  }
                                  if (name === 'totalIncome') {
                                    return [formatNumber(Number(value)), '年収入'];
                                  }
                                  if (name === 'totalAssets') {
                                    return [formatNumber(Number(value)), '総資産'];
                                  }
